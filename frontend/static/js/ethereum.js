...
let previousEthPrice = null; // 存储上一次的ETH价格
    ...
    fetch(`/api/latest_prices?_t=${timestamp}`, {
        ...
        .then(response => {
            if (response.success && response.data) {
                displayEthereumPrice(response.data);
            } else {
                throw new Error('获取数据失败');
            }
        })
        .catch(error => {
            console.error('获取以太坊价格时出错:', error);
            setStatus('获取价格数据失败', 'error');
        });
}

// 显示以太坊价格
function displayEthereumPrice(data) {
    const ethPriceElement = document.getElementById('ethPrice');
    if (!ethPriceElement) return;
    
    // 查找ETH数据
    const ethData = data.find(item => item.symbol === 'ETH');
    
    if (ethData) {
        const currentPrice = parseFloat(ethData.price);
        const priceChange = parseFloat(ethData.change_24h);
        const changeClass = priceChange >= 0 ? 'positive' : 'negative';
        const changeIcon = priceChange >= 0 ? '📈' : '📉';
        
        // 检查价格是否发生变化
        let priceChangeDirection = '';
        if (previousEthPrice !== null && previousEthPrice !== currentPrice) {
            priceChangeDirection = currentPrice > previousEthPrice ? 'price-up' : 'price-down';
            
            // 添加闪烁动画类
            ethPriceElement.classList.add('price-flash', priceChangeDirection);
            
            // 移除动画类
            setTimeout(() => {
                ethPriceElement.classList.remove('price-flash', 'price-up', 'price-down');
            }, 1000);
        }
...
function loadEthereumCharts() {
    ...
    fetch(`/api/eth_data?timeframe=${currentTimeframe}&limit=100&_t=${timestamp}`, {
        ...
        .catch(error => {
            console.error('获取以太坊图表数据时出错:', error);
            setStatus('获取图表数据失败', 'error');
        });
}

// 显示以太坊图表
function displayEthereumCharts(data) {
    // 显示价格图表
                    ...
                    callbacks: {
                        label: function(context) {
                            ...
                            return [
                                ...
                                `${status} (平均: ${averageVolatility.toFixed(2)}%)`
                            ];
                        }
                    }
                }
